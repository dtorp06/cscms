...
class Cstpl extends CI_Model{
    ...
    public function plub_list($row, $id, $fid='id', $page=1, $ids=array(), $return = FALSE, $tpl='list.html', $type='lists', $field='', $t='', $k='', $d='', $fidetpl=array()){
	    if(empty($ids)) $ids = $id;
        ...
        if($return || !$this->Cscache->get($cache_id)){ //缓存是否存在
			...
			if(!empty($page_arr) && !empty($page_arr[2])){
				...
				$sqlstr=$this->Csskins->cscms_sql($page_arr[1][0],$page_arr[2][0],$page_arr[0][0],$page_arr[3][0],$sort,$ids);
				//总数量
    ...
    public function home_list($row, $op='', $page=1, $tpl='index.html', $t='', $ids='', $fid='', $sql='', $return = FALSE, $field='user', $fidetpl=array(), $onetpl=array(),$hitsurl=''){
        ...
        if($return || !$this->Cscache->get($cache_id)){ //缓存是否存在
			...
			if(empty($row['skins'])){
				...
				if(defined('MOBILE')){
					$skinfile = 'mobile'.FGF.'home'.FGF.str_replace('/', FGF, $row['skins']).PLUBPATH.FGF.'index.html';
				}else{
					$skinfile = 'pc'.FGF.'home'.FGF.str_replace('/', FGF, $row['skins']).PLUBPATH.FGF.'index.html';
				}
