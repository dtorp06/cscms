...
var cscms = {
	...
	init  : function(sign){
						...
						if(data.error==0){
							...
							if(r.length>0){
	                            $.each(r, function(i,row){
	                                ...
	                                $("#type").append('<input type="checkbox" lay-skin="primary" name="type[]" title="'+row.name+'" value="'+row.name+'" '+chk+' />&nbsp;');
	                          	}); 
	                        }
                          	form.render('checkbox');
						}else{
		...
		window.onload=function(){
			if(!cscms.element) cscms.element = layui.element();
			if(!cscms.layer) cscms.layer = layui.layer;
			if(!cscms.laydate) cscms.laydate = layui.laydate;
			if(!cscms.upload) cscms.upload = layui.upload;
			if(sign==1){//默认加载边栏
