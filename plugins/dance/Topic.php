...
class Topic extends Cscms_Controller {
	...
	public function index($fid='id',$page = 0){
		...
		//装载模板并输出
        $Mark_Text=$this->Cstpl->plub_list($row,0,$fid,$page,'',false,'topic.html','topic','','歌曲专辑','歌曲专辑','',$zdy);
	}
	...
	public function show($fid='id',$id=0){
        ...
        $this->Cstpl->plub_show('topic',$row,$ids,false,'topic-show.html',$row['name'].' - '.L('dance_22'),$row['name'],'','',$zdy,$hitslink);
	}
}
