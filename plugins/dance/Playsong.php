...
class Playsong extends Cscms_Controller {
	...
	public function rand(){
	    ...
	    $callback = $this->input->get('callback',true);
        $lists=array();
        $result=$this->db->query("select id,singerid,name,tid,fid,purl,sc,lrc from ".CS_SqlPrefix."dance order by rand() desc LIMIT 30");
		$j=0;
		foreach ($result->result() as $row) {
		    $lrc=str_checkhtml($row->lrc);
            if(empty($lrc)) $lrc='0';
	...
	public function cais(){
        ...
        $result2=$this->db->query("select id,singerid,name,tid,fid,purl,sc,lrc from ".CS_SqlPrefix."dance where cid=".$row2->cid." order by rand() desc LIMIT 10");
		$j=0;
