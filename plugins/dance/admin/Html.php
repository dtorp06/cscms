...
class Html extends Cscms_Controller {
	...
	public function topicshow_save(){
        ...
        foreach ($query->result_array() as $row) {
			...
			//生成
			write_file(FCPATH.$Htmllink,$Mark_Text);
			echo "<font style=font-size:10pt;>生成歌曲专辑：<font color=red>".$row['name']."</font>成功：<a href=".$Htmllinks." target=_blank>".$Htmllinks."</a></font><br/><script>document.getElementsByTagName('BODY')[0].scrollTop=document.getElementsByTagName('BODY')[0].scrollHeight;</script>";
			ob_flush();flush();
