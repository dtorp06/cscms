...
class Lists extends Cscms_Controller {
	...
	public function init(){
        ...
        $this->Csdb->get_update('dance_list',$id,$edit);
        getjson('',0);
	}
	...
	public function plsave(){
		...
		foreach ($ids as $id) {
            ...
            $this->Csdb->get_update('dance_list',$id,$data);
		}
        $info['url'] = site_url('dance/admin/lists').'?v='.rand(1000,9999);
        $info['msg'] = '恭喜你，操作成功';
        getjson($info,0);
	}
	...
	public function edit(){
		...
		if($id==0){
		    ...
		    if(!$row) exit('记录不存在...2秒后返回<script>setTimeout(function(){history.back()},2000)</script>');
            $data['id']=$row->id;
            $data['fid']=$row->fid;
            $data['yid']=$row->yid;
            $data['xid']=$row->xid;
	...
	public function save(){
		...
		if($id==0){ //新增
            $this->Csdb->get_insert('dance_list',$data);
		}else{
            $this->Csdb->get_update('dance_list',$id,$data);
		}
        $info['url'] = site_url('dance/admin/lists').'?v='.rand(1000,9999);
	...
	public function del(){
		...
		$this->Csdb->get_del('dance_list',$ids,'fid');
		$this->Csdb->get_del('dance_list',$ids);
        $info['url'] = site_url('dance/admin/list').'?v='.rand(1000,9999);
        getjson($info,0);
	}
}
