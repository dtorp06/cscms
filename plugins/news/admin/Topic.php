...
class Topic extends Cscms_Controller {
	...
	public function index(){
        ...
        $base_url = site_url('news/admin/topic')."?yid=".$yid."&key=".$key."&sort=".$sort."&tid=".$tid."&page=";
    ...
    public function init($ac){
        ...
        $this->Csdb->get_update('news_topic',$id,$edit);
        getjson('',0);
    }
	...
	public function save(){
		...
		if($id==0){ //新增
            $this->Csdb->get_insert('news_topic',$data);
		}else{
            $this->Csdb->get_update('news_topic',$id,$data);
		}
        $info['url'] = site_url('news/admin/topic');
        getjson($info,0);
	}
	...
	public function del(){
		...
		$this->Csdb->get_del('news_topic',$ids);
        $info['url'] = site_url('news/admin/topic').'?v='.rand(1000,9999);
        getjson($info,0);
	}
}
