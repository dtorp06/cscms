...
class Lists extends Cscms_Controller {
    ...
    public function init(){
        ...
        $this->Csdb->get_update('news_list',$id,$edit);
        getjson('',0);
    }
	...
	public function plsave(){
		...
		foreach ($ids as $id) {
            ...
            $this->Csdb->get_update('news_list',$id,$data);
		}
        $info['msg'] = '恭喜您，操作成功~!';
        $info['url'] = site_url('news/admin/lists').'?v='.rand(1000,9999);
        getjson($info,0);
	}
	...
	public function save(){
		...
		if($id==0){ //新增
             $this->Csdb->get_insert('news_list',$data);
		}else{
             $this->Csdb->get_update('news_list',$id,$data);
		}
        $info['url'] = site_url('news/admin/lists').'?v='.rand(1000,9999);
	...
	public function del(){
		...
		$this->Csdb->get_del('news_list',$ids,'fid');
		$this->Csdb->get_del('news_list',$ids);
        $info['url'] = '恭喜您，删除成功~!';
        $info['url'] = site_url('news/admin/lists').'?v='.rand(1000,9999);
        getjson($info,0);
	}
