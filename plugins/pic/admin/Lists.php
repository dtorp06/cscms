...
class Lists extends Cscms_Controller {
    ...
    public function init(){
        ...
        $this->Csdb->get_update('pic_list',$id,$edit);
        getjson('',0);
    }
	...
	public function plsave(){
		...
		foreach ($ids as $id) {
            ...
            $this->Csdb->get_update('pic_list',$id,$data);
		}
        $info['msg'] = '恭喜您，操作成功~!';
        $info['url'] = site_url('pic/admin/lists').'?v='.rand(100,999);
        getjson($info,0);
	}
	...
	public function save(){
		...
		if($id==0){ //新增
            $this->Csdb->get_insert('pic_list',$data);
		}else{
            $this->Csdb->get_update('pic_list',$id,$data);
		}
        $info['msg'] = '恭喜您，操作成功~!';
	...
	public function del(){
		...
		$this->Csdb->get_del('pic_list',$ids,'fid');
		$this->Csdb->get_del('pic_list',$ids);
        $info['url'] = site_url('pic/admin/lists').'?v='.rand(100,999);
        getjson($info,0);
	}
}
