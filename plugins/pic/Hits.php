...
class Hits extends Cscms_Controller {
	...
	public function ids($id=0){
		$id = intval($id);   //ID			
		$row=$this->Csdb->get_row('pic_type','rhits,zhits,yhits,hits',$id);
		if(!$row){
		   exit();
		}
		//增加人气
		...
		if(!empty($str)){
		    $this->db->query("update ".CS_SqlPrefix."pic_type set ".implode(',',$str));
		}
	}
	...
	public function dt($op,$id=0,$type=''){
		...
		$row=$this->Csdb->get_row('pic_type',$op,$id);
		if(!$row){
