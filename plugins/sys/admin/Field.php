...
class Field extends Cscms_Controller {
	...
	public function index(){
		...
		if (is_file(CSCMS.'sys/Cs_Field.php')) {
		    $field = require_once(CSCMS.'sys/Cs_Field.php');
		    if(isset($field[$dir])){
	...
	public function edit(){
		...
		if($zd==''){
			...
			if (is_file(CSCMS.'sys/Cs_Field.php')) {
			    $field_res = require_once(CSCMS.'sys/Cs_Field.php');
	...
	function save(){
		...
		if (is_file(CSCMS.'sys/Cs_Field.php')) {
			$arrs = require_once(CSCMS.'sys/Cs_Field.php');
		}else{
			$arrs = array();
		}
		if(!empty($old_zd)){
			unset($arrs[$dir][$old_zd]);
		}
		$arrs[$dir][$data['zd']] = str_encode($data);
		arr_file_edit($arrs,CSCMS.'sys/Cs_Field.php');
		$info['url'] = site_url('field').'?dir='.$dir.'&v='.rand(100,999);
		$info['msg'] = L('plub_14');
		getjson($info,0);
	}
