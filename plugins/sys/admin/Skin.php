...
class Skin extends Cscms_Controller {
		    ...
		    foreach ($dir_arr as $t) {
			    ...
			    if (is_dir($path.$t)) {
					...
					if (file_exists($confiles)){
						...
						$pic = Web_Path.'tpl/'.$ac.'/'.$op.'/'.$t.'/preview.jpg';
                        if (!file_exists('.'.$pic)) $pic=Web_Path.'packs/images/skins.jpg';
						if($ac=='mobile'){
							if($op=='user'){
	...
	public function down(){
        ...
        $ac=intval($this->input->get_post('ac'));
        $op=intval($this->input->get_post('op'));
